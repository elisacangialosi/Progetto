...
#Gold Standard Set - II Tentativo
##Gold Standard 40 pairs review/Response
```{r}
goldStandardSet <- it_reviewResponse %>% select(-review_title, -stay_date, -review_score, -helpfulvote)
goldStandardSet$group <- gsub("([[:alpha:]]+)","",goldStandardSet$reviewID) #create a variable that identifies the review/response pairs
goldStandard2 <- goldStandardSet %>% arrange(group) # I order the dataset by the group, the objective is to select random group and run the validation of individual sentences as well as reviews of complete pairs of review response
goldStandard2$index <- c(1:length(goldStandard2$reviewID)) #I create a new column that uniquely identifies each sentence
x <- goldStandard2 %>% distinct(group) #create a vector with unique group number, I will randomly select group for my validation
set.seed(1234)
s <- sample(x$group,250)#random sample of 500 groups on which we will create our golden set
goldSetlabel <- filter(goldStandard2,group %in%s) # final dataset subject to manual labeling

goldSetlabelFinal <- goldSetlabel %>% 
  select(fullText, reviewID, index) %>% 
  bind_rows(
    goldSetlabel %>% 
      transmute(fullText = response)
...
write.csv(goldSetlabelFinal, "/users/elisacangialosi/Desktop/LouisianaStateUniversity/IISemester/Progetto/Data/GoldStandardSet/Prova2/Gold40/goldStandard40.csv", row.names = TRUE ,na = " " )

goldSetFinal <- goldStandard40[sample(1:nrow(goldStandard40)), ]

goldSetBind <- rbind(goldStandard250, goldStandard10)
uniqueGoldSet <- goldSetBind %>% distinct()

write.csv(goldSetFinal, "/users/elisacangialosi/Desktop/LouisianaStateUniversity/IISemester/Progetto/Data/GoldStandardSet/Prova2/RgoldStandard40.csv", row.names = TRUE ,na = " " )
```
