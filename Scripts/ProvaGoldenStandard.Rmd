...
set.seed(0500)
s10 <- sample(x$group,10)#random sample of 10 groups on which we will create our golden set
...
x <- goldenSet2 %>% distinct(group) #create a vector with unique group number, I will randomly select group for my validation
set.seed(0700)
s <- sample(x$group,40)#random sample of 80 groups on which we will create our golden set
...
join1 <- inner_join(rater1_10clean,rater2_10clean, by = "id")
join2 <- inner_join(join1 ,rater3_10clean, by = "id")
join3 <- inner_join(join2 ,rater4_10clean, by = "id")
dummy_tot10 <- inner_join(join3, rater5_10clean, by = "id")

#create totals
dummy_tot2_10 <- dummy_tot10 %>% group_by(id) %>%  mutate(tot_loc = sum(loc_dummy.x, loc_dummy.y, loc_dummy.x.x, loc_dummy.y.y, loc_dummy), tot_ser = sum(ser_dummy.x, ser_dummy.y, ser_dummy.x.x, ser_dummy.y.y, ser_dummy), tot_greet = sum(greet_dummy.x, greet_dummy.y, greet_dummy.x.x, greet_dummy.y.y, greet_dummy), tot_room = sum(room_dummy.x, room_dummy.y, room_dummy.x.x, room_dummy.y.y, room_dummy), tot_amenit = sum(amenit_dummy.x, amenit_dummy.y, amenit_dummy.x.x, amenit_dummy.y.y, amenit_dummy), tot_food = sum(food_dummy.x, food_dummy.y, food_dummy.x.x, food_dummy.y.y, food_dummy), tot_value = sum(value_dummy.x, value_dummy.y, value_dummy.x.x, value_dummy.y.y, value_dummy), tot_unclass = sum(unclass_dummy.x, unclass_dummy.y, unclass_dummy.x.x, unclass_dummy.y.y, unclass_dummy))
...
#Fleiss Kappa for complete set = 0.86691
```

#Import 40 gold Standard
